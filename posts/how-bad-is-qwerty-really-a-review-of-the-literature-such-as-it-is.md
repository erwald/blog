...
My current model on the RSI risk is something like this. The evidence has made me update toward the risk of developing serious RSI from keyboard use being small. I had a pretty strong prior, based on what I've heard from other programmers and on my own experience, that it often causes RSI. I still think it's a risk, but not as large as I did before, though with lots of uncertainty (80% confidence interval would be 2%-20% chance of developing keyboard RSI over a lifetime for the aforementioned target group). I think key layout is a minor part of the ergonomic harms of keyboard use; keyboard type (i.e. ergonomics of design) and posture/positioning both seem more important (70% confidence).
...
RSI is an umbrella term for injuries that occur due to "repetitive movements, awkward postures, sustained force, and other risk factors".[^10] The most common and costly form it takes is carpal tunnel syndrome (CTS), where the median nerve has gotten compressed where it travels through the wrist, leading to pain, discomfort, numbness, etc.[^11] QWERTY plausibly increases the risk of CTS because it involves rotating the wrist toward the pinky more often to reach characters at edge of the keyboard.[^12] (This is not the only ergonomic issue with keyboard use, but as far as I can tell, it is the only issue that varies for different layouts.) So once again Theory whispers, "QWERTY baaad." Is it?
...
(It's perhaps worth pointing out that, as risk factor for RSI, keyboard use is way less risky than some manual labor.[^15][^16] Most CTS patients are "industrial workers, females, and the elderly", not young male programmers.[^17] Reported CTS has decreased overall in the United States labor force, and appears in larger numbers in blue collar industries than in white collar industries even though there are over 50% more workers in the latter; mostly it seems that manufacturing work is to blame.[^18] I find this easy to believe; manufacturing work seems like really hard work, especially on the body.)
...
Alternative designs like Colemak and Dvorak were optimized for typing English, but many people do most of their typing in multiple languages. What should they (we, actually) do? QWERTY has lots of variants with special characters for different languages; alternative designs are usually one-size-fits-all solutions where special characters are typed via so-called dead keys (in other words, via multiple keys in succession). (Of course it's possible to customize one's key layout, but that's only an option for technical people with time on their hands, like me.)

I think the answer here is: it depends. For me, using Colemak for Swedish and German in addition to English has been fine, but I do most of my typing in English, and Swedish and German are both Germanic languages that have similar letter frequencies to English's and few additional characters. If you are sometimes writing Turkmen, say, you may have to switch between layouts if you want to use something like Colemak or Dvorak for English. There seem to be few modern layouts optimized specifically for non-English languages.
